...
### 2단계: API Key 구성 (개인 보안 영역)
프로젝트 루트 폴더에 존재하는 [local.properties](file:///c:/CookAndroid/Project/DotNote/local.properties) 파일을 열고 본인의 API Key를 기재합니다. (이 파일은 로컬 빌드용이므로 Git 및 깃허브에는 추적되지 않고 제외됩니다.)
```properties
sdk.dir=C\:\\CookAndroid\\SDK
